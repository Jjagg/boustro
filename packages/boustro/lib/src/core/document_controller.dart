...
class DocumentController extends ValueNotifier<List<ParagraphController>> {
  ...
  void clear() {
    final paragraphsCopy = [..._paragraphs];
    _paragraphs.clear();

    paragraphsCopy.forEach(
      (c) => _paragraphRemovedController.add(ParagraphRemovedEvent(c)),
    );
    paragraphsCopy.forEach((c) => c.dispose());
...
abstract class DocumentState {
  ...
  @protected
  bool tryMergeNext(int index) {
    if (index < 0 || index + 1 >= paragraphs.length) {
      return false;
    }

    final p1 = paragraphs[index];
    final p2 = paragraphs[index + 1];
    if (p1 is TextParagraphControllerMixin &&
        p2 is TextParagraphControllerMixin &&
        p1.runtimeType == p2.runtimeType) {
      final merged = p1.textController.attributedText
          .concat(AttributedText('\n'))
          .concat(p2.textController.attributedText);
      p1.textController.attributedText = merged;
      removeAt(index + 1);
      return true;
    }
...
class _MutableDocumentState extends DocumentState {
...
class _ControllerDocumentState extends DocumentState {
  _ControllerDocumentState(this._controller);

  final DocumentController _controller;
  List<ParagraphController> get paragraphs => _controller.paragraphs;
...
class DefaultDocumentRules extends DocumentRules {
  ...
  void enforce(DocumentState state) {
    ...
    var index = 0;
    while (index < state.paragraphs.length - 1) {
      if (!state.tryMergeNext(index)) index++;
    }
  }
}
